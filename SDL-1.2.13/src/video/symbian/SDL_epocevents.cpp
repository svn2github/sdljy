...
void EPOC_GenerateKeyEvent(_THIS,int aScanCode,int aIsDown)
{	
	...
	if(key.IsLower())
	{
		scancode=key.GetUpperCase();
	}
	else if(key.IsUpper())
	{
		SDL_PrivateKeyboard(SDL_PRESSED, TranslateKey( _this,EStdKeyLeftShift, &keysym));
	}
	...
	if(key.IsUpper())
	{
		SDL_PrivateKeyboard(SDL_RELEASED, TranslateKey( _this,EStdKeyLeftShift, &keysym));						
	}			
}
	...
	case TPointerEvent::EButton1Down:
		{					
			...
			default:
				{
					...
					{
						TChar key(iLastChar);
						if(key.IsAlpha() && !current_video->hidden->iShiftOn)
						{
							iLastChar = key.GetLowerCase();										
						}
					}
			...
			if(iLastChar!=-1)
			{
				...
				EPOC_GenerateKeyEvent(_this, iLastChar, ETrue);
	...
	case TPointerEvent::EButton1Up:
		{
			if(iLastChar!=-1)
			{
				SDLKey oldBackSpace = keymap[EStdKeyBackspace];
				keymap[EStdKeyBackspace]    = SDLK_BACKSPACE;
				EPOC_GenerateKeyEvent(_this, iLastChar, EFalse);
				iLastChar=-1;
				keymap[EStdKeyBackspace]    = oldBackSpace;
				return ETrue;
			}
...
void EPOC_InitOSKeymap(_THIS)
{
	...
	keymap[EStdKeyLeftFunc]     = SDLK_UNKNOWN;
...
TUint32 hid_to_SDL[256] =
    ...
    /*  48  */ '¨','\'',000,']',000,000,',','.','-',SDLK_CAPSLOCK,SDLK_F1,SDLK_F2,SDLK_F3,SDLK_F4,SDLK_F5,SDLK_F6,
    ...
    /*  224 */ SDLK_LCTRL,SDLK_LSHIFT,SDLK_LALT,'~',000,SDLK_RSHIFT,000,000,000,000,000,000,000,000,000,000,
    /*  240 */ 000,000,000,000,000,000,000,000,000,000,000,000,000,000,000,000
    };

TUint32 hid_to_SDL_shifted[256] = 
    {
    /*  00  */ 000,000,000,000,'A','B','C','D','E','F','G','H','I','J','K','L',
    /*  16  */ 'M','N','O','P','Q','R','S','T','U','V','W','X','Y','Z','!','"',
    /*  32  */ '#','¤','%','&','/','(',')','=',SDLK_RETURN,SDLK_ESCAPE,SDLK_BACKSPACE,SDLK_TAB,SDLK_SPACE,'?','\\','}',
    /*  48  */ '^','*',000,'[',000,000,';',':','_',SDLK_CAPSLOCK,SDLK_F1,SDLK_F2,SDLK_F3,SDLK_F4,SDLK_F5,SDLK_F6,
    /*  64  */ SDLK_F7,SDLK_F8,SDLK_F9,SDLK_F10,SDLK_F11,SDLK_F12,SDLK_PRINT,SDLK_SCROLLOCK,SDLK_BREAK,SDLK_INSERT,SDLK_HOME,SDLK_PAGEUP,SDLK_DELETE,SDLK_END,SDLK_PAGEDOWN,SDLK_RIGHT,
    /*  80  */ SDLK_LEFT,SDLK_DOWN,SDLK_UP,SDLK_NUMLOCK,SDLK_KP_DIVIDE,SDLK_KP_MULTIPLY,SDLK_KP_MINUS,SDLK_KP_PLUS,SDLK_KP_ENTER,SDLK_KP1,SDLK_KP2,SDLK_KP3,SDLK_KP4,SDLK_KP5,SDLK_KP6,SDLK_KP7,
    /*  96  */ SDLK_KP8,SDLK_KP9,SDLK_KP0,SDLK_KP_PERIOD,'<',000,000,SDLK_KP_EQUALS,SDLK_F13,SDLK_F14,SDLK_F15,000,000,000,000,000,
    /*  112 */ 000,000,000,000,000,000,000,000,000,000,000,000,000,000,000,000,
...
void CHIDEventMonitor::RunL()
    ...
    switch (hidEvent.Type())
                ...
                case EEventButtonUp:
                    switch(mouse->iValue)
						...
						case EMouseButtonSide:
							EPOC_GenerateKeyEvent(current_video, SDLK_RETURN, down);							
							break;
						case EMouseButtonForward:
							EPOC_GenerateKeyEvent(current_video, SDLK_PAGEDOWN, down);
							break;
						case EMouseButtonBack:
							EPOC_GenerateKeyEvent(current_video, SDLK_PAGEUP, down);
							break;
                        default:
                            break;
                        }
        ...
        case THIDEvent::EKeyEvent:
            ...
            int hid_code = hid_to_SDL[key->ScanCode()];
            switch (key->Type())
                {
                case EEventHIDKeyUp:
                    if (shift_down)
                        {
                        hid_code = hid_to_SDL_shifted[key->ScanCode()];
                        }
                    if (hid_code == SDLK_LSHIFT)
                        {
                        shift_down = EFalse;
                        }                 
                    EPOC_GenerateKeyEvent(current_video, hid_code, EFalse);
                    break;
...
//                    Com_Printf( "EEventHIDKeyDown, code %d\n",key->ScanCode());
					...
					EPOC_GenerateKeyEvent(current_video, hid_code, ETrue);
                    break;
                default:
                    break;
                }
