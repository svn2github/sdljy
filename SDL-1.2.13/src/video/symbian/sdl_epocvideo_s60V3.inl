...
{	
	...
	if((Private->iSX0Mode & ESX0Portrait))
	{
		...
		if((Private->EPOC_DisplaySize.iWidth - Private->iModeSize.iWidth)>KVirtualKeyboardSpace)
		{
			xStretch = (TReal)(Private->EPOC_DisplaySize.iWidth-KVirtualKeyboardSpace)/(TReal)Private->iModeSize.iWidth;
			Private->iKeyboardRect = TRect(Private->EPOC_DisplaySize.iWidth-KVirtualKeyboardSpace,0, Private->EPOC_DisplaySize.iWidth,Private->EPOC_DisplaySize.iHeight);
			if(xStretch > 2)
			{
				Private->iKeyboardRect.iTl.iX-=(xStretch-2)*(TReal)(Private->iModeSize.iWidth);			
			}			
		}
		else if((Private->EPOC_DisplaySize.iHeight-Private->iModeSize.iHeight)>KVirtualKeyboardSpace)
		{
			yStretch = (TReal)(Private->EPOC_DisplaySize.iHeight-KVirtualKeyboardSpace)/(TReal)Private->iModeSize.iHeight;
			Private->iKeyboardRect = TRect(0, Private->EPOC_DisplaySize.iHeight-KVirtualKeyboardSpace, Private->EPOC_DisplaySize.iWidth,Private->EPOC_DisplaySize.iHeight);
			if(yStretch > 2)
			{
	...
	else
	{
		...
		if((Private->EPOC_DisplaySize.iHeight - Private->iModeSize.iWidth)>KVirtualKeyboardSpace)
		{
			xStretch = (TReal)(Private->EPOC_DisplaySize.iHeight-KVirtualKeyboardSpace)/(TReal)Private->iModeSize.iWidth;
			Private->iKeyboardRect = TRect(0,Private->EPOC_DisplaySize.iHeight-KVirtualKeyboardSpace, Private->EPOC_DisplaySize.iWidth,Private->EPOC_DisplaySize.iHeight);
			if(xStretch > 2)
			{
		...
		else if((Private->EPOC_DisplaySize.iWidth-Private->iModeSize.iHeight)>KVirtualKeyboardSpace)
		{
			yStretch = (TReal)(Private->EPOC_DisplaySize.iWidth-KVirtualKeyboardSpace)/(TReal)Private->iModeSize.iHeight;
			Private->iKeyboardRect = TRect(Private->EPOC_DisplaySize.iWidth-KVirtualKeyboardSpace, 0, Private->EPOC_DisplaySize.iWidth,Private->EPOC_DisplaySize.iHeight);
			if(yStretch > 2)
			{
	...
	// This limits the stretching if other side is shrunk.
	if(xStretch < 1 && yStretch > 1)
	{
		yStretch = 1;
	}
	
	if(yStretch < 1 && xStretch > 1)
	{
		xStretch = 1;
	}
	...
	if(Private->iModeSize != TSize(0,0))
	{
		...
		if((_this->hidden->iSX0Mode & ESX0Portrait))// Portrait mode
		{
			...
			{		
				if(_this->hidden->iSX0Mode & ESX0Stretched)
				{
					if(!_this->hidden->iIs240Mode || _this->hidden->EPOC_ScreenSize.iHeight>=240)
					{		
		...
		else if(_this->hidden->iSX0Mode & ESX0Stretched)
		{
			...
			if(_this->hidden->iIs240Mode)
			{
				for(loop=0;loop<240;loop++)
				{
					...
					{
						i240StartTable[loop]=(Private->EPOC_ScreenSize.iWidth-1)-pos;				
					}
					else
		...
		else // Full landscape.. i.e 1-1 cropped or not
		{
			...
			for(loop=0;loop<240;loop++)
			{		
				if(!_this->hidden->iIs240Mode || _this->hidden->EPOC_ScreenSize.iWidth>=240)
				{		
					if(_this->hidden->iSX0Mode&ESX0Flipped)
					{
				...
				{
					TReal factor = (TReal)_this->hidden->EPOC_ScreenSize.iWidth/(TReal)_this->hidden->iModeSize.iHeight;
...
inline void EPOC_S60LandscapeStretchUpdate(_THIS, int numrects, SDL_Rect *rects)
{
	...
	if(Private->iNeedFullRedraw )
	{
		...
		for(TInt loop=0;loop<numrects;loop++)
		{
			...
			{
				rect.x = Private->EPOC_DisplaySize.iHeight-gScaleXPos[rect.x];
				rect.y = gScaleYPos[rect.y];
				rect.w = (gScaleXPos[rect.w])+1;
				rect.h = (gScaleYPos[rect.h])+1;
...
static void EPOC_DirectUpdate(_THIS, int numrects, SDL_Rect *rects)
{
	...
	if(_this->hidden->iSX0Mode & ESX0Portrait)
	{
		if((_this->hidden->iSX0Mode&ESX0Stretched && !Private->EPOC_ShrinkedHeight /*&& _this->screen->w<=Private->EPOC_ScreenSize.iWidth*/))
		{
	...
	if(((_this->hidden->iSX0Mode&ESX0Stretched) && !Private->EPOC_ShrinkedHeight/* && _this->screen->w<=Private->EPOC_ScreenSize.iHeight*/))
	{
