...
CApaDocument* CSDLApp::CreateDocumentL()
{
	CEBasicDoc* doc = NULL;
...
TFileName CSDLApp::GetExecutablePath()
{
	return TheBasicAppUi->GetExecutablePath();
}

char* CSDLApp::GetExecutablePathCStr()
{
 ...
 */
void CSDLApp::PreInitializeAppL()
{
}
...
void CEBasicAppUi::MultitapTimeoutL()
{
	...
	iMultitapTimer->Cancel();
	iReleaseKeys.Append(current_video->hidden->iCurrentChar);
	SetTextInputState(EFalse); // Allow numbers
	EPOC_GenerateKeyEvent(current_video,current_video->hidden->iCurrentChar, ETrue);
...
void CEBasicAppUi::ReleaseMultitapKeysL()
{
	...
	{
		SetTextInputState(EFalse); // Allow numbers
		EPOC_GenerateKeyEvent(current_video, iReleaseKeys[0], EFalse);
		SetTextInputState(ETrue);
	...
	case '0':
		{
			iMultitapTimer->Cancel();
			SetTextInputState(EFalse); // Allow numbers
			EPOC_GenerateKeyEvent(current_video,' ',on);
			SetTextInputState(ETrue);		
			return EKeyWasConsumed;
		}
	case EStdKeyNkp1:
	case '1':
		{
			iMultitapTimer->Cancel();
			SetTextInputState(EFalse); // Allow numbers
			EPOC_GenerateKeyEvent(current_video,current_video->hidden->iCurrentChar,on);
			SetTextInputState(ETrue);		
		}		
		return EKeyWasConsumed;		
	case EStdKeyNkp2:
...
#endif
#if defined (UIQ3) || defined (S60) || defined (S60V3)	
			case KRightButtonCode: // Joystick button 2 or keyboard space
				if(current_video->hidden->iInputMode == EJoystick)
					current_video->hidden->iJoystickStatus[EJoyBUT2]=on;
				else if(current_video->hidden->iInputMode == EMouseMode)
					SDL_PrivateMouseButton(on?SDL_PRESSED:SDL_RELEASED, SDL_BUTTON_RIGHT, 0, 0);					
				break;				
			case KLeftButtonCode2:// Joystick button 2 or keyboard return
			case KLeftButtonCode1:			
#endif
				if(current_video->hidden->iInputMode==EJoystick)
...
void CEBasicAppUi::SDLStartL()
{
	...
	TRAPD(exiterr, main(1,iExecutableParams));
#if defined (UIQ) || defined (S80) || defined (S90) || defined (S60)
